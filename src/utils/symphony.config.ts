...
import { 
  SYMPHONY_GLOBAL_DIR, SYMPHONY_ENV, 
  SYMPHONY_DEPLOY_REPO, SYMPHONY_DEPLOY_DIR, 
  SYMPHONY_DASHBOARD_REPO, SYMPHONY_DASHBOARD_DIR, 
  SYMPHONY_DEPLOYMENT_ENV, HIDE_CDK_OUTPUT,
  AWS_DIR, HOME } from "./constants.js";
import { prompt, exec, spawnChild, camelCamse } from "./helpers.js";
dotenv.config({path: SYMPHONY_GLOBAL_DIR});

import { SymphonyConfig, AwsCredentials } from "../types.js"

const assertSymphonyDirectory = async (spinner: Spinner) => {
...
const assertDeployRepo = async (spinner: Spinner) => {
...
const assertDashboardRepo = async(spinner: Spinner) => {
  process.chdir(SYMPHONY_GLOBAL_DIR);
...
const assertAwsCredentials = async (spinner: Spinner) => {
...
const writeConfig = (newConfig : SymphonyConfig) => {
  if ((newConfig.AWS_ACCOUNT || newConfig.AWS_REGION) && fs.existsSync(SYMPHONY_DEPLOY_DIR + "/.env")) {
    const currentDeploymentConfig = readDeploymentConfig() as {AWS_REGION?: string, AWS_ACCOUNT?: string};
...
const writeDeploymentConfig = (newConfig: any) => {
  const config = Object.entries(newConfig).map(pair => pair.join("=")).join("\n");
...
const assertSymphonyConfig = async(spinner: Spinner) => {
...
const confirmUpdateRequest = (prop: string): string => {
...
const updateSymphonyConfig = async(newConfig: SymphonyConfig) => {
...
const installAwsCLI = async(spinner: Spinner) => {
  try {
    ...
    spinner.succeed("Successfully installed AWS CLI")
     ...
     throw new Error(
        "AWS CLI could not be installed. For further guidance, see https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html"
      );
  }
}

const assertAwsCli = async(spinner: Spinner) => {
  try {
    await exec('aws --version')
...
const deployInfrastructure = async (spinner: Spinner, project="Symphony") => {
  ...
  spinner.start(
    "Deploying AWS infrastructure with CDK. Grab a coffee ☕️. This process may take some time..."
  );

  await exec(
    `cdk deploy --outputs-file ${HIDE_CDK_OUTPUT} --require-approval never`
  );
...
const readConfig = (): SymphonyConfig => {

  if (!fs.existsSync(SYMPHONY_ENV)) return {};
...
const assertBootstrap = async(region: string) => {
  try {
...
export { 
  ...
  assertAwsCredentials, assertSymphonyConfig,
  assertDeployRepo, assertDashboardRepo,
  
  assertAwsCli, assertAwsCdk,

  deployInfrastructure,

  readConfig, readAwsCredentials,
  readDeploymentConfig, 
  writeDeploymentConfig, writeDashboardConfig,
  assertBootstrap,
